...
def deserialize_game(state):
    ...
    game.ai_moves = state.get('ai_moves')
    game.max_moves = state.get('max_moves')
    return game
...
def guess_song(state, song_name, artist_name):
    ...
    # Check limit
    current_moves = len(game.history) + len(game.guessed_live_ids)
    if game.max_moves and current_moves >= game.max_moves:
        return state, "Game Over! You have run out of moves.", format_history(game), ""
    ...
    current_moves = len(game.history) + len(game.guessed_live_ids)
    if game.max_moves:
        msg += f"\nMoves: {current_moves}/{game.max_moves}"
        if current_moves >= game.max_moves:
